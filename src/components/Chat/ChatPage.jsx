  ...
  return (
    <div className="flex flex-col h-full max-h-[calc(100vh-4rem)]">
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-border">
        ...
        <button onClick={clearChat} className="p-1.5 rounded hover:bg-accent text-muted-foreground hover:text-foreground" title="New conversation">
          <RefreshCw size={14} />
        </button>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3">
        {messages.length === 0 && (
          <div className="text-center text-muted-foreground mt-12">
            ...
            <p className="text-xs mt-2 text-muted-foreground">💡 Your assistant remembers previous conversations — no need to repeat context.</p>
          </div>
        )}
        {messages.map((m, i) => (
      ...
      <form onSubmit={sendMessage} className="p-3 border-t border-border">
        <div className="flex gap-2">
          <input
            ...
            className="flex-1 bg-card border border-border rounded-lg px-3 py-2 text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:border-primary"
            disabled={sending}
            autoFocus
          />
          <button
            ...
            className="bg-primary hover:bg-primary/90 disabled:bg-muted disabled:text-muted-foreground text-primary-foreground rounded-lg px-3 py-2 transition-colors"
          >
            <Send size={16} />
          </button>
        </div>
